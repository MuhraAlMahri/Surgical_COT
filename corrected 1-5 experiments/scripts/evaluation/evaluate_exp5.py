...
def generate_prediction(model, processor, device, image, prompt_text, max_new_tokens=128):
...
def evaluate_model(model, processor, device, test_data, image_base_path, max_samples=None):
    ...
    for item in tqdm(test_data, desc="Evaluating"):
        ...
        try:
            ...
            prompt_stage1 = f"{question}"
            pred1 = generate_prediction(model, processor, device, image, prompt_stage1)
            
            # Stage 2: Build on Stage 1
            prompt_stage2 = f"{question}\n\nBased on the initial observation: {pred1}\n\nWhat is your refined answer?"
            pred2 = generate_prediction(model, processor, device, image, prompt_stage2)
            
            # Stage 3: Final answer
            prompt_stage3 = f"{question}\n\nInitial observation: {pred1}\nRefined answer: {pred2}\n\nWhat is your final, most concise answer?"
            pred3 = generate_prediction(model, processor, device, image, prompt_stage3)
